...
export default function MetricCard({ name, value, history, unit, icon = "activity" }: MetricCardProps) {
      ...
      <CardContent className="pb-4">
        <div className="flex justify-between items-end">
          <div>
            ...
            <p
              className={`text-sm font-medium ${percentChange >= 0 ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}`}
            >
              {percentChange >= 0 ? "+" : ""}
          ...
          <div className="h-16 w-24">
            <canvas ref={canvasRef} width={96} height={64} className="w-full h-full"></canvas>
          </div>
        </div>
      </CardContent>
    </Card>
